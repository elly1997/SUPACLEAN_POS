-- Migration Script: Separate Items from Services
-- This script creates the items table and updates the schema to support:
-- 1. Items = Laundry items (Shirts, Suits, etc.) with base prices
-- 2. Services = Delivery types (Express, Regular) with multipliers
-- 3. Branch-specific pricing for items
-- 4. Update orders to reference items
...
CREATE TABLE IF NOT EXISTS items (
    ...
    name TEXT NOT NULL,
    description TEXT,
    category TEXT NOT NULL DEFAULT 'general', -- 'gents', 'ladies', 'general'
    base_price DECIMAL(10,2) NOT NULL DEFAULT 0,
    service_type TEXT NOT NULL DEFAULT 'Wash, Press & Hanged', -- Service type description
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Step 2: Create Branch Item Prices Table (for branch-specific pricing)
...
-- Add foreign key to items table (will be added after items table exists)
-- We'll add this constraint in a separate step after items table is created

-- Step 3: Update Services Table to represent Delivery Types
-- Services should now be: "Regular Service" (multiplier 1.0), "Express Service < 8HRS" (multiplier 2.0), "Express Service < 3HRS" (multiplier 3.0)
-- We'll update the existing services or create new ones

-- Step 4: Add item_id to orders table (do this after items table is created)
...
-- Then add the foreign key constraint (only if items table exists)
DO $$ 
BEGIN
...
-- Step 5: Populate Items from the price list provided
-- Gents Items
...
-- Step 6: Update Services to represent Delivery Types
-- Delete old services and create new delivery-type services
...
-- Step 7: Add foreign key constraint for branch_item_prices -> items
-- This must be done after items table is created and populated
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'items') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                       WHERE constraint_name = 'branch_item_prices_item_id_fkey') THEN
...
COMMENT ON TABLE items IS 'Laundry items with base prices (Shirts, Suits, etc.)';
COMMENT ON TABLE branch_item_prices IS 'Branch-specific pricing for items';
COMMENT ON TABLE services IS 'Delivery service types with multipliers (Regular, Express)';
